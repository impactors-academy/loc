...
export default function PropertiesPage() {
  ...
  const load = () => {
    setLoading(true)
    api.admin.properties.list().then(setItems).finally(() => setLoading(false))
  }
  useEffect(load, [])

  const handleDelete = async (slug: string, title: string) => {
    if (!confirm(`Delete "${title}"? This cannot be undone.`)) return
    await api.admin.properties.delete(slug)
    load()
  }
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-heading text-2xl font-semibold text-loc-night">Properties</h1>
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-border">
              <tr>{["Title","Type","Country","Tier","Actions"].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-loc-stone uppercase tracking-wide">{h}</th>)}</tr>
            </thead>
